...
public class ClientGlobal {
    ...
    public static final String SCONF_KEY_CONNECT_TIMEOUT = "ctimeout";
    public static final String CONF_KEY_NETWORK_TIMEOUT = "network_timeout";
    public static final String SCONF_KEY_NETWORK_TIMEOUT = "ntimeout";
    ...
    public static final String CONF_KEY_HTTP_TRACKER_HTTP_IP = "http.tracker_http_ip";
    ...
    public static final String PROP_KEY_HTTP_TRACKER_HTTP_IP = "fastdfs.http_tracker_http_ip";
    ...
    public static String g_tracker_http_ip = "";
     ...
     */
    public static void initByFile(String conf_filename) throws IOException, MyException {
        IniFileReader iniReader = new IniFileReader(conf_filename);
        initByFileReader(iniReader);
    }

    /**
     * load global variables from input stream
     *
     * @param in input stream
     */
    public static void initByStream(InputStream in) throws IOException, MyException {
        IniFileReader iniReader = new IniFileReader("");
        iniReader.loadFromStream(in);
        initByFileReader(iniReader);
    }

    /**
     * load global variables from input file reader
     *
     * @param iniReader ini file reader
     */
    public static void initByFileReader(IniFileReader iniReader) throws IOException, MyException {
        String[] szTrackerServers;
        String[] parts;

        g_connect_timeout = iniReader.getIntValue(CONF_KEY_CONNECT_TIMEOUT, -1);
        if (g_connect_timeout < 0) {
            g_connect_timeout = iniReader.getIntValue(SCONF_KEY_CONNECT_TIMEOUT, -1);
        }
        if (g_connect_timeout < 0) {
        ...
        g_network_timeout = iniReader.getIntValue(CONF_KEY_NETWORK_TIMEOUT, -1);
        if (g_network_timeout < 0) {
            g_network_timeout = iniReader.getIntValue(SCONF_KEY_NETWORK_TIMEOUT, -1);
        }
        if (g_network_timeout < 0) {
        ...
        g_tracker_http_ip = iniReader.getStrValue(CONF_KEY_HTTP_TRACKER_HTTP_IP);
        g_tracker_http_port = iniReader.getIntValue(CONF_KEY_HTTP_TRACKER_HTTP_PORT, 80);
    ...
    public static void initByProperties(Properties props) throws IOException, MyException {
        ...
        if (trackerServersConf == null) {
            throw new MyException(String.format("configure item %s is required but null", PROP_KEY_TRACKER_SERVERS));
        } else {
            String key = trackerServersConf.trim();
            if (key.length() > 0) {
                throw new MyException(String.format("configure item %s is required but length is zero", PROP_KEY_TRACKER_SERVERS));
            }
        }
        ...
        String httpTrackerHttpIPConf = props.getProperty(PROP_KEY_HTTP_TRACKER_HTTP_IP);
        String httpTrackerHttpPortConf = props.getProperty(PROP_KEY_HTTP_TRACKER_HTTP_PORT);

        if (connectTimeoutInSecondsConf != null) {
            String key = connectTimeoutInSecondsConf.trim();
            if (key.length() > 0) {
                g_connect_timeout = Integer.parseInt(key) * 1000;
            }
        }
        if (networkTimeoutInSecondsConf != null) {
            String key = networkTimeoutInSecondsConf.trim();
            if (key.length() > 0) {
                g_network_timeout = Integer.parseInt(key) * 1000;
            }
        }
        if (charsetConf != null) {
            String key = charsetConf.trim();
            if (key.length() > 0) {
                g_charset = key;
            }
        }
        if (httpAntiStealTokenConf != null) {
            String key = httpAntiStealTokenConf.trim();
            if (key.length() > 0) {
                g_anti_steal_token = Boolean.parseBoolean(key);
            }
        }
        if (httpSecretKeyConf != null) {
            String key = httpSecretKeyConf.trim();
            if (key.length() > 0) {
                g_secret_key = key;
            }
        }
        if (httpTrackerHttpIPConf != null) {
            String key = httpTrackerHttpIPConf.trim();
            if (key.length() > 0) {
                g_tracker_http_ip = key;
            }
        }
        if (httpTrackerHttpPortConf != null) {
            String key = httpTrackerHttpPortConf.trim();
            if (key.length() > 0) {
                g_tracker_http_port = Integer.parseInt(key);
            }
        }
    }
    ...
    public static String getG_tracker_http_ip() {
        return g_tracker_http_ip;
    }

    public static void setG_tracker_http_ip(String tracker_http_ip) {
        ClientGlobal.g_tracker_http_ip = tracker_http_ip;
    }
    ...
    public static String configInfo() {
        ...
        return "{"
                ...
                + "\n  g_tracker_http_ip = " + g_tracker_http_ip
